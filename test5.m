...
glcm = graycomatrix(G, 'NumLevels', 8, 'Offset', [0 1; -1 1; -1 0; -1 -1], 'Symmetric', true);

glcmFeatures = cad_glcm_features(glcm);

figure;
subplot(2, 2, 1), imshow(G), title('Grayscale Image');
subplot(2, 2, 2), imshowpair(Gmag, Gdir, 'montage'), title('Gradient Mag and Dir');
subplot(2, 2, 3), imshow(F,[]), title('Gaussian Filtering');
